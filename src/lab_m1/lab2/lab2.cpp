...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_A"], shaders["VertexColor"], glm::vec3(2, 0, -1), glm::vec3(1));

    // TODO(student): Draw the mesh that was created with `CreateMesh()`
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(-2, 0, -1), glm::vec3(1));
    ...
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1, 1, 1), glm::vec3(1));
